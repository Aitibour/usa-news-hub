...
window.resolveArticleImage = function(article) {
  // If article already has a specific image that's not a generic pool fallback, keep it
  if (article.image && article.image.includes("unsplash.com/photo-") && !article.autoGenerated) {
    return article.image;
  }
  const pool = IMAGE_POOL[article.section] || IMAGE_POOL.politics;
  // Hash the article id to get consistent, unique index
  let hash = 0;
